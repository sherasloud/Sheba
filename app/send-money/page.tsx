...
function SendMoneyContent() {
  ...
  const handleNextStep = async () => {
    if (step === 1) {
      ...
      // Check if recipient exists, if so get their name
      try {
  ...
  if (!isVerified) {
    return <VerificationRequired title="Send Money" />
  }
    ...
    <div className="mobile-page">
      ...
      {step === 1 && (
        <div className="mobile-content">
          <div className="text-xl font-bold mb-2">টাকা পাঠান</div>
          <div className="text-gray-600 mb-6 text-sm">ফ্রি চার্জ - কোন ফি নেই!</div>

          <div className="mb-2 flex items-center">
            <div className="mr-2">📞</div>
            <div className="text-sm font-medium">ফোন নম্বর</div>
          </div>
      ...
      {step === 2 && (
        <div className="mobile-content">
          ...
          <div className="text-gray-600 mb-2 text-sm">প্রাপক: {recipientName || phoneNumber}</div>
          {fromQR && (
            ...
            <div className="text-sm text-blue-800">
              ...
              {amount && Number(amount) > 0 && (
                <div className="flex justify-between items-center mt-1 pt-1 border-t border-blue-200">
                  <span>পাঠানোর পর:</span>
                  <span className="font-bold text-green-600">৳ {(balance - Number(amount)).toLocaleString()}</span>
                </div>
              )}
            </div>
          </div>
      ...
      {step === 3 && (
        <div className="mobile-content">
          ...
          <div className="text-gray-600 mb-1 text-sm">পরিমাণ: ৳ {Number(amount).toLocaleString()}</div>
          <div className="text-gray-600 mb-1 text-sm">প্রাপক: {recipientName || phoneNumber}</div>
          ...
          <div className="flex space-x-2 mt-auto">
            ...
            <button className="flex-1 mobile-button" onClick={handleSendMoney} disabled={isTransferring}>
              {isTransferring ? "প্রক্রিয়াকরণ..." : "টাকা পাঠান"}
            </button>
          </div>
        </div>
      )}
